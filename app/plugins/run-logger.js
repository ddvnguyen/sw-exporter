...
module.exports = {
    ...
    init (proxy, config)
    {
        setTimeout(() =>
        {
            app.showNotification("Test Notification");
        }, 2000);
                ...
                if (config.Config.Plugins[this.pluginName].enabled) {
                    ...
                    switch (command) {
                        ...
                        case "BattleDungeonStart":
                            if (this.repeatBattle[wizardID])
                                clearTimeout(this.repeatBattle[wizardID].waitReRun);
                            break;

                        case 'BattleDungeonResult_V2':
                        case 'BattleScenarioResult':
                        case 'BattleDungeonResult':
                            if (req.auto_repeat > 0)
                                this.checkEndRepeatBattle(req, resp);
    ...
    checkEndRepeatBattle (req, resp)
    {
        let wizardName = resp.wizard_info.wizard_name;
        let wizardID = resp.wizard_info.wizard_id;

        if (resp.win_lose === 2) {
            app.showNotification(wizardName + " Repeat Battle End Because Lost");
            return;
        }

        if (req.auto_repeat === 10) {
            app.showNotification(wizardName + " Repeat Battle End After 10 Run");
        }
        else {
            if (!this.repeatBattle[wizardID])
                this.repeatBattle[wizardID] = {};

            this.repeatBattle[wizardID].waitReRun = setTimeout(() =>
            {
                app.showNotification(wizardName + " Repeat Battle End By Others Reason");
            }, 20000);
        }
    },
    ...
    getArtifactData (artifact)
    {
        const artifactDrop = {
            drop: 'Artifact',
            set: `${gMapping.artifact.types[artifact.type]} (${artifact.type === 1 ? gMapping.monster.attributes[artifact.attribute] : gMapping.monster.archetypes[artifact.unit_style]
                })`,
